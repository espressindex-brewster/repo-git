...
async function main() {
  if (!orarioConsentito()) {
    ...
    console.error(`Fuori orario (${ora} IT). Chiama tra 08:00–11:30 o 15:00–17:30.`)
    process.exit(1)
  }

  const barIds = process.argv.find(a => a.startsWith('--ids='))?.split('=')[1]?.split(',') ?? []
  if (barIds.length === 0) {
    console.error('Uso: npm run batch:calls -- --ids=uuid1,uuid2,...')
    process.exit(1)
  }
  ...
  console.log(`\nBatch chiamate — ${bars?.length ?? 0} bar\n${'─'.repeat(50)}`)
